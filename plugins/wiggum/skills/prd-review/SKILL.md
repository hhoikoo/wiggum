...
3. Fetch all review comments on the PR:
   ...
   ```bash
   gh api repos/{owner}/{repo}/pulls/<pr-number>/comments --jq '.[] | {id, author: .user.login, body, path, line: .original_line, in_reply_to_id, created_at}'
   ```

4. Identify which comments need attention. A comment needs attention if it has no reply from the current agent (i.e., no later comment in the same thread authored by the agent). Both top-level comments and follow-up replies from reviewers can contain new feedback -- do not skip a comment just because `in_reply_to_id` is set. The test is whether the comment itself has been responded to, not whether it is a top-level comment.

## Phase 2: Triage Comments

5. For each unaddressed review comment, assign a confidence score (0-100%) representing how likely the comment identifies a real issue that should be addressed.
...
6. For comments where action is `incorporate`: launch a subagent (or batch related comments into one subagent) to:
...
7. Launch a subagent to compose reply text for each comment based on the triage action:
...
8. Post replies to each comment:
   ```bash
   gh api repos/{owner}/{repo}/pulls/<pr-number>/comments/<comment-id>/replies -f body="<reply>"
   ```

9. Fetch unresolved review threads and resolve the ones that were addressed:
   ```bash
...
10. If any PRD files were modified, commit and push. Delegate to `/wiggum:commit`:
