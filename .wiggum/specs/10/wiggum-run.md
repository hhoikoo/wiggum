...
4. **Hybrid plan completion: max iterations with early exit on structured JSON signal.** Plan mode runs up to `max_plan_iterations` (default 5) but exits early when the claude agent outputs structured JSON indicating the plan is complete. Each plan iteration prompt instructs claude to end its output with a JSON block (`{"status": "complete"}` or `{"status": "in_progress"}`) that the CLI parses. If the CLI detects `"status": "complete"`, the plan loop exits immediately. Otherwise, iteration continues up to the max. This avoids wasting iterations when the plan converges early (typical in 1-3 iterations) while capping runtime for plans that never stabilize. The structured signal is more reliable than diffing file contents between iterations or looking for magic sentinel strings.
...
| `src/wiggum/runner.py` (new) | Core loop logic: `run_plan()`, `run_build()`, `run_combined()`. Iteration loop, subprocess invocation, file updates. Plan mode parses structured JSON from claude output to detect early completion. Validates `.wiggum/implementation/<ticket-num>/` exists at startup, creates skeleton plan/progress files if missing. |
| `src/wiggum/templates/plan.md` (new) | Plan mode prompt template using `string.Template` `$variable` placeholders. Numbered-step structure from ralph loop guide. Includes instruction for claude to output structured JSON status (`{"status": "complete"}` or `{"status": "in_progress"}`) at end of response. Loaded via `importlib.resources`. |
...
| `src/wiggum/subprocess_util.py` (new) | `invoke_claude()` wrapping `Popen` with stdin prompt, stdout capture, stderr passthrough. Always passes `--dangerously-skip-permissions`. Returns structured result including stdout for JSON status parsing. `terminate_subprocess()` with SIGTERM/SIGKILL sequence. |
...
| `tests/test_prompts.py` (new) | Template loading via `importlib.resources`, `string.Template` substitution, prompt rendering with and without quality commands, plan prompt includes JSON status instruction. |
...
- [ ] Given plan mode is running and claude outputs `{"status": "in_progress"}` on each iteration, when the loop executes, then exactly `max_plan_iterations` iterations run (default 5), each spawning a separate claude process.
- [ ] Given plan mode is running and claude outputs `{"status": "complete"}` on iteration 2, when the loop checks the JSON output, then the plan loop exits after iteration 2 without running remaining iterations.
- [ ] Given plan mode is running and claude output contains no parseable JSON status, when the loop checks for the signal, then the iteration is treated as `in_progress` and the loop continues to the next iteration (graceful degradation).
...
| Fixed iteration count only for plan completion (no early exit) | Wastes iterations when the plan converges early (typical in 1-3 iterations). Structured JSON output from the agent is a reliable completion signal that avoids fragile heuristics like file diffing while keeping the max as a safety cap. |
...
- How should the implementation ticket number be communicated to `wiggum run`? Options: CLI argument, derived from the current branch name, or read from a file in `.wiggum/implementation/`. This depends on the `feature-work-on` skill's design.
- Should the plan completion JSON signal use a fenced code block (easier to parse from mixed output) or a bare JSON line at the end of stdout? The choice affects parsing robustness when claude includes JSON in its reasoning.
